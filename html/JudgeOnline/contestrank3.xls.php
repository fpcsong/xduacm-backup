...
if ($rows_cnt>0){
	...
	header ( "content-disposition:   attachment;   filename=contest3".$cid."_".$title.".xls" );
}
...
$sql="SELECT 
	personinfo.username,personinfo.realname,personinfo.StuNum,solution.result,solution.num,solution.in_date 
		FROM 
			(select * from solution where solution.contest_id='$cid' and num>=0) solution 
		inner join personinfo 
		on personinfo.username=solution.user_id 
	ORDER BY personinfo.username,in_date";
//echo $sql;
$result=mysql_query($sql);
$user_cnt=0;
$user_name='';
...
while ($row=mysql_fetch_object($result)){
	...
	if (strcmp($user_name,$n_user)){
		$user_cnt++;
		$U[$user_cnt]=new TM();
		$U[$user_cnt]->user_id=$row->username;
                $U[$user_cnt]->realname=$row->realname;
		$U[$user_cnt]->StuNum=$row->StuNum;
...
echo "<table border=1><tr><td>Rank<td>Student_id<td>Name<td>Solved<td>Time";
for ($i=0;$i<$pid_cnt;$i++)
	echo "<td>$PID[$i]";
echo "</tr>";
...
for ($i=0;$i<$user_cnt;$i++){
	...
	echo "<td>'$uusn";
	if(strpos($_SERVER['HTTP_USER_AGENT'],'MSIE')){
		$U[$i]->realname=iconv("utf8","gbk",$U[$i]->realname);
	}
	echo "<td>".$U[$i]->realname."";
	echo "<td>$usolved";
	echo "<td>";
	
	
	echo $U[$i]->time>0?intval($U[$i]->time):0;
	for ($j=0;$j<$pid_cnt;$j++){
		echo "<td>";
		if(isset($U[$i])){
